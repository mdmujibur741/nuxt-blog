<template>
  ...
  >
    ...
    <AppLayoutNavigation v-if="!isMobile" class="p-4 bg-white shadow mb-1 mt-1" /> 
...
<!-- <template>
  <VaLayout 
    :top="{ fixed: true, order: 2 }"
    :left="{ fixed: true, absolute: breakpoints.smDown, order: 1, overlay: breakpoints.smDown && !isSidebarMinimized }"
    @leftOverlayClick="isSidebarMinimized = true"
  >
    <template #top>
      <AppNavbar :is-mobile="isMobile" />
...
// Update onResize to only minimize sidebar for small screens (smDown)
const onResize = () => {
  isSidebarMinimized.value = breakpoints.smDown // Only collapse on small screens and below
  isMobile.value = breakpoints.smDown
  isTablet.value = breakpoints.mdDown
  sidebarMinimizedWidth.value = isMobile.value ? '0' : '4.5rem'
  sidebarWidth.value = isTablet.value ? '100%' : '16rem'
...
onBeforeRouteUpdate(() => {
  if (breakpoints.smDown) {
    // Collapse sidebar after route change for Mobile (smDown)
    isSidebarMinimized.value = true
  }
})
